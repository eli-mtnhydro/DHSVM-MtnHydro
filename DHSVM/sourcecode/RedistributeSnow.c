                      ...
                      TOPOPIX **TopoMap, MAPSIZE *Map)
{
  ...
  float MaxErode, MinDownwindElev, DownwindSlope, SaltScale, QsaltToSusp;
  float *QsuspLocal;
  ...
  //     WindMap[y][x].nIters--;
  //   
  //   // if (x==35 && (y==165 || y==170 || y==180))
  //   //   printf("x %d y %d nIter %d\n",x,y,WindMap[y][x].nIters);
  // }
  ...
  // if (x==34 && y==170 && (Qsalt + Qsusp + QsaltIncoming + QsuspIncoming) > 0.0)
  //   printf("\nRefSpeed: %.1f qSalt: %.1e qSaltIn: %.1e qSusp: %.1e qSuspIn: %.1e Deposition: %.1e SWE: %.2e\n",
  //          RefWindSpeed, Qsalt, QsaltIncoming, Qsusp, QsuspIncoming, WindMap[y][x].WindDeposition, LocalSnow->Swq);
    ...
    if (WindMap[y][x].WindDirFrac[0][k] > 0.0) {
      ...
      if (valid_cell(Map, nx, ny) && INBASIN(TopoMap[ny][nx].Mask)) {
        ...
        if (DownwindSlope >= 0.0) {
          /* Reduce upward saltation on steep slopes and add deposition to current cell */
          // SaltScale = (ThreshSaltSlope - MIN(DownwindSlope, ThreshSaltSlope)) / ThreshSaltSlope;
          // WindMap[y][x].WindDeposition += (1.0 - SaltScale) * WindMap[y][x].Qsalt * WindMap[y][x].WindDirFrac[0][k] / DepthConversion;
          SaltScale = 1.0;
        } else {
      ...
      MinDownwindElev += TopoMap[ny][nx].Dem * WindMap[y][x].WindDirFrac[0][k];
  }
  }
  ...
  for (l = 0; l < NWINDLAYERS; l++) {
    if (WindMap[y][x].Qsusp[l] > 0.0 || (l == 0 && QsaltToSusp > 0.0)) {
      ...
      // KinEnergy = DeltaZ /  TravelTime;
      // KinEnergy *= KinEnergy;
      // KinEnergy /= 2.0;
      // UpperZ += ABSVAL(DeltaZ) * MIN(2.0, WindMap[y][x].TurbulenceK[l] / KinEnergy);
      // LowerZ -= ABSVAL(DeltaZ) * MIN(2.0, WindMap[y][x].TurbulenceK[l] / KinEnergy);
      
      // if (WindMap[y][x].Qsusp[l] > 1e3 && y==170 && x==34)
      //   printf("l %d Zscale: %.2f XYscale: %.2f TravelTime: %.0f LiftRatio: %0.2f\n",
      //          l,WindMap[y][x].WindSpeedZ[l],WindMap[y][x].WindSpeedXY[l],TravelTime,LiftRatio);
      
      // LandingFrac = 0.0;
      // ExportFrac = 1.0;
      
      /* Account for amount potentially landing in current grid cell */
      if (UpperZ < MinDownwindElev) {
        LandingFrac = 1.0;
        ExportFrac = 0.0;
      }
      else if (LowerZ < MinDownwindElev) {
        LandingFrac = (MinDownwindElev - LowerZ) / (UpperZ - LowerZ);
      ...
      // if (WindMap[y][x].Qsusp[l] > 1e4 && y==171 && (x==32 || x==34 || x==38))
      //   printf("x %d l %d Susp: %.1e SWE: %.1f DeltaZ: %.0f DeltaXY: %.0f Height: %.0f TravelTime: %.0f ExportFrac: %0.2f\n",
      //          x,l,WindMap[y][x].Qsusp[l],LocalSnow->Swq,DeltaZ,
      //          TravelTime * RefWindSpeed * WindMap[y][x].WindSpeedXY[l],
      //          (UpperZ + LowerZ) / 2.0 - TopoMap[y][x].Dem - DeltaZ,
      //          TravelTime,ExportFrac);
      // if (WindMap[y][x].Qsusp[l] > 1e2 && y==170 && x==34)
      //   printf("l %d Susp: %.1e DZ: %.0f DXY: %.0f H: %.0f TravelT: %.0f Export: %.2f Land: %.2f Upper: %.0f Lower: %.0f DEMn: %.0f\n",
      //          l,WindMap[y][x].Qsusp[l],DeltaZ,
      //          TravelTime * RefWindSpeed * WindMap[y][x].WindSpeedXY[l],
      //          (UpperZ + LowerZ) / 2.0 - TopoMap[y][x].Dem - DeltaZ,
      //          TravelTime,ExportFrac,LandingFrac,UpperZ,LowerZ,MinDownwindElev);
            ...
            if (valid_cell(Map, nx, ny) && INBASIN(TopoMap[ny][nx].Mask)) {
              ...
              if (LayerFrac < 1.0) {
                for (j = 0; j < NWINDLAYERS; j++) {
                  ...
                  // if (ny==170 && nx==34 && LayerFrac > 0.0 &&
                  //     WindMap[y][x].Qsusp[l] * WindMap[y][x].WindDirFrac[l][k] * ExportFrac * LayerFrac > 1e2)
                  //   printf("x: %d y: %d l: %d --> %d Overlap: %.1f Flux: %.2e\n",
                  //          x,y,l,j,OverlapWidth,
                  //          WindMap[y][x].Qsusp[l] * WindMap[y][x].WindDirFrac[l][k] * ExportFrac * LayerFrac);
                }
                           ...
                           WINDPIX **WindMap, TOPOPIX **TopoMap)
{
  ...
  // Z0eff = Z0_SNOW * (1 + (WindMap[y][x].TurbulenceK[0] - 0.1) / 0.1);
  ...
  for (l = 0; l < NWINDLAYERS; l++) {
    if (WindMap[y][x].Qsusp[l] > (1e-9 * DepthConversion)) {
      ...
      if (WindMap[y][x].IsSnowing == TRUE &&
        ...
        // cSublimation *= (float) Dt;
        cSublimation *= TravelTime;
        
        // if (x== && y==167)
        //   printf("l %d Susp %.3e RefWind: %.3f WindXY: %.3f Time: %.3f SubFrac %.3f Snowing: %d Rh: %.3f\n",
        //          l,WindMap[y][x].Qsusp[l],RefWindSpeed,WindMap[y][x].WindSpeedXY[l],TravelTime,cSublimation,
        //          WindMap[y][x].IsSnowing,(WindMap[y][x].IsSnowing == TRUE ? 1.0 : LocalMet->Rh / 100.0));
